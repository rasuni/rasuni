...
public final class TitanCollector
{
	...
	{
		rasuni.org.apache.log4j.helpers.LogLog.debug(debugEnabled, quietMode, out, s1 + s2 + s3);
	}
	...
	private static void selectAndConfigure(URL url, Runnable isNull)
	{
		...
		else
		{
			...
			try
			{
				OptionConverter.selectAndConfigure(url, rasuni.org.apache.log4j.helpers.OptionConverter.getSystemProperty(LogManager.CONFIGURATOR_CLASS_KEY, System.security, System.props, null, LogLog.g_debugEnabled, LogLog.g_quietMode, System.out),
	...
	public static void configureLogOptions(boolean java1, boolean ignoreTCL, String resource, Runnable isNull)
	{
		...
		try
		{
			if (java1 || ignoreTCL)
			{
				...
				ClassLoader classLoader = Loader.class.getClassLoader(System.security);
				if (classLoader == null)
				{
					// It
			...
			else
			{
				...
				if (classLoader != null)
				{
					...
					if (url1 != null)
					{
						...
						try
						{
							...
							OptionConverter.selectAndConfigure(url,
									rasuni.org.apache.log4j.helpers.OptionConverter.getSystemProperty(LogManager.CONFIGURATOR_CLASS_KEY, System.getSecurityManager(), props, null, LogLog.g_debugEnabled, LogLog.g_quietMode, System.out),
									LogManager.getLoggerRepository());
						}
						catch (NoClassDefFoundError e)
						{
					...
					else
					{
						// We could not find resource. Ler us now try with the
						// classloader that loaded this class.
						classLoader = Loader.class.getClassLoader(/*System.getSecurityManager()*/);
						if (classLoader != null)
						{
							rasuni.org.apache.log4j.helpers.LogLog.debug(LogLog.g_debugEnabled, LogLog.g_quietMode, System.out, "Trying to find [" + resource + "] using " + classLoader + " class loader.");
	...
	private static final int LOG4J_DEBUG_HASH = Strings.hashCode("log4j.debug");

	private static final int LOG4J_DEBUG_PHASH = LOG4J_DEBUG_HASH & 0x7FFFFFFF;

	private static final int LOG4J_CONFIG_DEBUG_HASH = Strings.hashCode("log4j.configDebug");

	private static final int LOG4J_CONFIG_DEBUG_PHASH = LOG4J_CONFIG_DEBUG_HASH & 0x7FFFFFFF;
	 ...
	 */
	@SuppressWarnings("deprecation")
	public static void run(SecurityManager security, Properties props, boolean debugEnabled, boolean quietMode, PrintStream out, final String db, final Iterable<Iterable<String>> roots, final boolean checkAccuracy, Iterable<String> playListPath)
	{
		try
		{
			if (security == null)
			{
				String debugKey = PropertiesUtil.get(props, LOG4J_DEBUG_PHASH, LOG4J_DEBUG_HASH, "log4j.debug", () ->
				{
					try
					{
						return PropertiesUtil.get(props, LOG4J_CONFIG_DEBUG_PHASH, LOG4J_CONFIG_DEBUG_HASH, "log4j.configDebug", () ->
						{
							return null;
						});
					}
					...
					{ // MS-Java throws com.ms.security.SecurityExceptionEx
							...
							{
								out.println("log4j: Was not allowed to read system property \"log4j.configDebug\".");
							}
							return null;
						}
					});
			...
			{
				security.checkPropertyAccess("log4j.debug");
				String debugKey = PropertiesUtil.getProperty("log4j.debug", props);
				if (debugKey == null)
				{
					try
					{
						security.checkPropertyAccess("log4j.configDebug");
						debugKey = PropertiesUtil.getProperty("log4j.configDebug", props);
					}
					catch (Throwable e)
					{ // MS-Java throws com.ms.security.SecurityExceptionEx
						if (debugEnabled && !quietMode)
		...
		{ // MS-Java throws com.ms.security.SecurityExceptionEx
			rasuni.org.apache.log4j.helpers.LogLog.debug(LogLog.g_debugEnabled, LogLog.g_quietMode, System.out, "Was not allowed to read system property \"" + "log4j.debug" + "\".");
			String debugKey = null;
			debugKey = rasuni.org.apache.log4j.helpers.OptionConverter.getSystemProperty("log4j.configDebug", System.security, System.props, null, LogLog.g_debugEnabled, LogLog.g_quietMode, System.out);
			if (debugKey != null)
			{
				LogLog.g_debugEnabled = OptionConverter.toBoolean(debugKey, true);
			}
								...
								while (irl.hasNext())
								{
									...
									case RECORDING:
									{
										...
										while (ir.hasNext())
										{
											...
											case ARTIST_RECORDING_PRODUCER:
												break;
											default:
												TitanCollector.fail();
											}
					...
					if (iterator.hasNext())
					{
						...
						case PLACE:
						{
							...
							if (plFile.exists())
							{
								try
								{
									Assert.expect(java.nio.file.Files.readAllLines(plFile.toPath()).iterator().next().equals("# place/" + placeId));
								}
								catch (IOException e)
								{
									throw new RuntimeException(e);
						...
						case ARTIST:
						{
							...
							if (plFile.exists())
							{
								try
								{
									...
									Assert.expect(Strings.startsWith("# artist/", line));
						...
						case WORK:
						{
							...
							if (plFile.exists())
							{
								try
								{
									Iterator<String> iLine = java.nio.file.Files.readAllLines(plFile.toPath(), StandardCharsets.UTF_8).iterator();
									String line = iLine.next();
									Assert.expect(Strings.startsWith("# work/", line));
	...
	public static void main_deprecated()
	{
		...
		run(System.security, System.props, LogLog.g_debugEnabled, LogLog.g_quietMode, System.out, "collection", roots, false, Arrays.asList("\\\\qnap", "Qmultimedia"));
	}
	...
	private static <EL extends EntityList, T> Iterator<T> iterator(final IExpression<EL, MetaData> entityList, final WebService<MetaData> musicBrainz, final Resource resource, final Parameter parameter, final IExpression<LinkedList<T>, EL> listExpr)
	{
		return new Iterator<T>()
		{
			...
			private void fetch()
			{
				_list = entityList.apply(musicBrainz.query(resource.getName(), parameter, new Parameter("limit", "100"), new Parameter("offset", String.valueOf(_offset))));
	 ...
	 *            the properties
	 * @param debugEnabled
	 *            true if debug log is enabled
	 * @param quietMode
	 *            quiet logging mode
	 * @param out
	 *            rhe out print stream
	 *
	 * @param db
	 *            the database name
	 * @param checkAccuracy
	 ...
	 *            the base entries
	 */
	public static void run(SecurityManager security, Properties props, boolean debugEnabled, boolean quietMode, PrintStream out, final String db, final boolean checkAccuracy, String[] baseEntries)
	{
		run(security, props, debugEnabled, quietMode, out, db, Arrays.asList((Iterable<String>) Arrays.asList(baseEntries)), checkAccuracy, Arrays.asList(baseEntries));
	}
}