...
{
	private int _rootKind;
	private SQLiteConnection _connection;
	private ITaskColumn _additionalColum;
	private final static IStatementAction<Void> EXPECT_NO_DATA = new IStatementAction<Void>()
	{
		@Override
	...
	public static void start(String name, ITaskHandler application, final ITaskColumn additionalColumn, IIndex taskIndex, ITableDef additionalTable, int rootKind, IDatabaseApplication[] updaters)
	{
		...
		try
		{
			...
			else
			{
				...
				{
					columns.add(new IColumnDef()
					{
						@Override
						public void visit(IColumnVisitor visitor)
						{
							visitor.definition(additionalColumn.getColumnName(), ColumnDef.STRING, additionalColumn.getConstraint());
						}
					});
				}
	...
	public Iterable<IRow> select(final Iterable<Column<?>> resultColumns, String joinSource, String whereExpression, Iterable<IValue> parameterValues, boolean limit, String orderColumn)
	{
		...
		return statement(selectSql(sql.toString(), joinSource, whereExpression, limit, orderColumn), parameterValues, new IStatementAction<Iterable<IRow>>()
		{
			@Override
			public Iterable<IRow> execute(final IStatement statement)
			{
				LinkedList<IRow> result = new LinkedList<>();
				while (statement.step())
				{
					Object[] slots = new Object[count];
					Row row = new Row(slots, new IRecordType()
					{
						@Override
						public int getId(Column<?> column)
						{
							return idForColumn.get(column);
						}
					});
					result.add(row);
					int pos = 0;
